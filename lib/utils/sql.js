...
async function addAdmin(id,isMaster) { // 添加管理员
  let isExists = await isAdminExists(id);
  if(!isExists) { // 如果不存在这个管理员
    return new Promise((resolve,reject) => {
      let add = PushDatabase.prepare(`INSERT INTO admin VALUES ('${id}',${Boolean(isMaster)});`);
      add.run(() => {
        resolve('success');
      });
    });
...
async function addTask(id,type,name,guild,sub) { // 添加订阅
  ...
  if(isExists) { // 如果存在
    ...
    let subs = `["${guild}#${sub}"]`;
    return new Promise((resolve,reject) => {
      let add = PushDatabase.prepare(`INSERT INTO push VALUES('${id}',${type},'${name}','${subs}');`); // 追加订阅
      add.run(() => {
        resolve('success'); // 成功
      });
    });
...
async function removeTaskCompletely(id) { // 完全移除订阅
  let isExists = await isTaskExists(id);
  if(isExists) {
    return new Promise((resolve,reject) => {
      let remove = PushDatabase.prepare(`DELETE FROM push WHERE id = '${id}'`);
      remove.run(() => {
        resolve('success');
      });
    });
...
export { addAdmin,removeAdmin,getUserLevel,addTask,removeTask,isTaskSubscribed,removeTaskCompletely,getSubscribeList,getAllSubscribes }